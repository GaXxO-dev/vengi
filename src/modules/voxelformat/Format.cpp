...
bool Format::singleVolume() const {
	return core::getVar(cfg::VoxformatMerge)->boolVal();
}
...
bool PaletteFormat::loadGroups(const core::String &filename, const io::ArchivePtr &archive,
	...
	const bool createPalette = core::getVar(cfg::VoxelCreatePalette)->boolVal();
	if (!createPalette) {
...
int PaletteFormat::emptyPaletteIndex() const {
	...
	return core::getVar(cfg::VoxformatEmptyPaletteIndex)->intVal();
}
